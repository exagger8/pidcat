...
__version__ = '2.2.0-exagger8'

LOG_LEVELS = 'VDIWEF'
LOG_LEVELS_MAP = dict([(LOG_LEVELS[i], i) for i in range(len(LOG_LEVELS))])
parser = argparse.ArgumentParser(description='Filter logcat by package name')
parser.add_argument('package', nargs='*', help='Application package name(s)')
parser.add_argument('-w', '--tag-width', metavar='N', dest='tag_width', type=int, default=30, help='Width of log tag')
...
parser.add_argument('--timestamp', dest='timestamp', action='store_true', default=True,
                    help='Display the timestamp')
parser.add_argument('-hl', '--highlight', dest='highlight', type=str, help='Phrases to catch and highlight (a|b)')
...
TAGTYPES = {
    'V': colorize(' V ', fg=WHITE, bg=BLACK), # gray #BBBBBB
    'D': colorize(' D ', fg=BLACK, bg=GREEN), # green #04F900
    'I': colorize(' I ', fg=BLACK, bg=CYAN), # blue #0072F9
    'W': colorize(' W ', fg=BLACK, bg=YELLOW), # yellow #FFCE1D
    'E': colorize(' E ', fg=BLACK, bg=RED), # red #FF6B68
    'F': colorize(' F ', fg=BLACK, bg=RED), # red #FF6B68
}

MSG_COLOR_BY_TYPE = {
    'V': WHITE, # gray #BBBBBB
    'D': GREEN, # green #04F900
    'I': CYAN, # blue #0072F9
    'W': YELLOW, # yellow #FFCE1D
    'E': RED, # red #FF6B68
    'F': MAGENTA, # red #FF6B68
}
...
while adb.poll() is None:
    ...
    if level in MSG_COLOR_BY_TYPE:
        fgColor = MSG_COLOR_BY_TYPE[level]
        message = colorize(message, fgColor)
        tColor = termcolor(fgColor)
        for phrase in highlights:
            message = message.replace(phrase, colorize(phrase, BLACK, fgColor) + tColor)
